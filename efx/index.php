<?php
  $url = (isset($_GET['i']) && $_GET['i'] && strlen($_GET['i']) > 5) ? str_replace("http:/","http:/",str_replace('https:/', 'https:/', $_GET['i'])) : 'https://jsbot.cantelope.org/uploads/1cUypu.mp4';
?>
<!DOCTYPE html>
<html>
  <head>
    ...
    <script>
      ...
      Draw=()=>{
        ...
        if(src[0]){
          ...
          for(i=0;i<ref.data.length;i+=4){
            red = ref.data[i+0]
            green = ref.data[i+1]
            blue = ref.data[i+2]
            lum = (red+green+blue)/3
            if(lum<10) ref.data[i+3]=0
          }
  ...
  if(
    ...
    srcurl.toLowerCase().indexOf('.webp') !== -1 ||
